...
async function basicAuth(req, res, next) {
    try {
        ...
        // Read MongoDB - FIXED: Use consistent collection name
        ...
        console.error("BasicAuth error:", error);
...
app.get('/', (req, res) => {
    res.json({ 
        message: 'PWS API Server is running',
        endpoints: ['/signin', '/signup', '/users', '/products', '/cart', '/orders'],
        timestamp: new Date().toISOString()
    });
});

//sign in - ENHANCED with better error handling
app.post('/signin', async (req, res) => {
    try {
        ...
        // Find the user by email - FIXED: Use consistent collection name
...
app.post('/signup', async (req, res) => {
    try {
        const user = req.body;
        console.log('Signup attempt:', { email: user.email });
...
app.get('/checkpassword', (req, res) => {
    ...
    if (!password || !confirmPassword) {
        return res.status(400).json({ error: 'Missing password or confirmPassword' });
    }
...
app.put('/order-items/:id', async (req, res) => {
    try {
        ...
        console.error("PUT /order-items error:", error);
...
app.delete('/order-items/:id', async (req, res) => {
    try {
        ...
        console.error("DELETE /order-items error:", error);
